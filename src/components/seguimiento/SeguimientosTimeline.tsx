...
export default function SeguimientosTimeline({ items, activeId, onSelect }: Props) {
  ...
  // Días según mes (y año si quieres precisión de febrero)
  const days = React.useMemo(() => {
    if (!month) return [];
    const y = year ? Number(year) : 2024; // año neutro
      ...
      <div className="flex flex-wrap items-center justify-end gap-2">
        ...
        <select
          ...
          onChange={(e) => {
            ...
            setMonth(v);
            setDay(""); // reseteamos día cuando cambia mes
          }}
          className="rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs sm:text-sm font-medium text-slate-700 shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-500/60"
        >
          {MONTHS.map((m) => (
        ...
        {/* Día (solo cuando hay mes) */}
        <select
          value={day}
          onChange={(e) => setDay(e.target.value)}
          disabled={!month}
          className={`rounded-lg border px-3 py-1.5 text-xs sm:text-sm font-medium shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-500/60 ${
            month
