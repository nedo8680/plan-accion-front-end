...
export default function ImportSeguimientoFile({
  onImport,
  onOptionsFromApi,
  nombreEntidad,
}: Props) {
  ...
  // ===== Auto-check: consultar backend una vez al montar (cuando ya hay entidad)
  useEffect(() => {
    let mounted = true;
    ...
    const fetchReport = async () => {
      try {
        ...
        console.error(
          "ImportSeguimientoFile: error al llamar /reports/{nombre_entidad}",
          e
        );
        ...
        if (first) {
          ...
          setFileName(
            "Datos de ejemplo (sincronización externa no disponible)"
          );
          setTimeout(() => setFileName(""), 4000);
        }
      }
