...
export default function PlanesSidebar({
  ...
  // Número de días según el mes (y año si está definido, para febrero)
  const daysInMonth = useMemo(() => {
    if (!month) return 31; // valor por defecto, pero el select de día estará deshabilitado
    const m = parseInt(month, 10); // 1–12
    const y = year ? parseInt(year, 10) : 2001; // año neutro no bisiesto si no se eligió
    // new Date(y, m, 0) -> último día del mes `m`
  ...
  const filtered = useMemo(() => {
    ...
    return plans.filter((p) => {
      ...
      if (hasDateFilter) {
        ...
        if (day && dy !== day) return false;
      }

      return true;
    });
  }, [plans, q, year, month, day]);
